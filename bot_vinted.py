...
# --- CONFIGURATION ---
TOKEN = "MTQ5NDAwMTA2MjgxNzA0MjY3Nw.G0k3HS.hoi6giSkuNMuNDMtADcidIv4WMugTPjdvu1B9s" # Remplis ton token ici
...
class VintedItemView(ui.View):
    def __init__(self, item_url, negotiate_url):
        ...
        # Bouton Link: "Négocier" (ouvre la messagerie)
        self.add_item(ui.Button(
            label="💬 Négocier", 
            style=discord.ButtonStyle.link, 
            url=negotiate_url
...
class VintedBot(discord.Client):
    ...
    async def on_ready(self):
        ...
        # On récupère les cookies dès le démarrage
        ...
        try:
            ...
            for channel_id, url, name in filters:
                ...
                for item in items[:15]:  # On analyse les 15 derniers
                    ...
                    # --- EXTRACTION SÉCURISÉE ET CORRIGÉE DES DONNÉES ---
                    title = item.get('title', 'Sans titre')
                    
                    # 1. FIX: Extraction propre du prix
                    price_info = item.get('price', {})
                    if isinstance(price_info, dict):
                        raw_amount = price_info.get('amount', '??')
                        raw_currency = price_info.get('currency_code', '€')
                        try:
                            formatted_price = f"{float(raw_amount):.2f}"
                        except (ValueError, TypeError):
                            formatted_price = raw_amount
                    else:
                        formatted_price = price_info # Fallback
                        raw_currency = item.get('currency', '€')
                    
                    brand = item.get('brand_title', 'Inconnue')
                    size = item.get('size_title', 'N/A')
                    
                    # 2. ADD: Extraction de l'état
                    condition = item.get('status_title', 'Inconnu')
                    
                    photo = item.get('photo', {}).get('url')
                    item_url = f"https://www.vinted.fr/items/{item_id}"
                    
                    # URL de négociation (ouvre le chat)
                    ...
                    # Utilisation d'emoji pour un meilleur visuel
                    ...
                    # --- ADD: Création et envoi avec boutons ---
...
# --- COMMANDES SLASH POUR TOUS LES UTILISATEURS ---
bot = VintedBot()
...
async def add_filter(interaction: discord.Interaction, nom_du_filtre: str, url_vinted: str):
    if "vinted.fr" not in url_vinted:
        await interaction.response.send_message("❌ URL Vinted invalide. Copie l'URL depuis ton navigateur.", ephemeral=True)
        return
    ...
    await bot.db.commit()
    await interaction.response.send_message(f"✅ Filtre '**{nom_du_filtre}**' activé dans ce salon ! Je t'enverrai les nouveautés.", ephemeral=False)
